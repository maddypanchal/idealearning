...
public partial class Employee_EditParentsSpeaks : System.Web.UI.Page
{
    #region "Variable Decleration"
    private clsMaster obj_News = new clsMaster();
    private DataSet DS;
    private int RecordStatus;
    private static int ParentsSpeaksId;
    private string myFileExtension;
    #endregion

    protected void Page_Load(object sender, EventArgs e)
    {
        ParentsSpeaksId = int.Parse(Request.QueryString["Sid"]);
        if (!IsPostBack)
        {
            FillGallery();
        }
    ...
    {
        obj_News.ParentsSpeaksId = ParentsSpeaksId;
        DS = obj_News.EditParentsSpeaks();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (FileUploadOnLocalComputer.HasFile)
        {
            if (myFileExtension == ".jpg" || myFileExtension == ".bmp" || myFileExtension == ".png" || myFileExtension == ".jpeg" || myFileExtension == ".JPG" || myFileExtension == ".gif")
            {
                if (FileUploadOnLocalComputer.PostedFile.ContentLength < 3000000)
                {
                    ...
                    obj_News.ImageName = Path.GetFileName("PS-" + FileNamekey);
                }
                else
                {
        ...
        DS = obj_News.UpdateParentsSpeaks();
        if (RecordStatus == 0)
        {
           //lblmsg.Text = "Record Saved!";
        }
        //clearform();
        Response.Redirect("ParentsSpeaksDetails.aspx");
    }
}